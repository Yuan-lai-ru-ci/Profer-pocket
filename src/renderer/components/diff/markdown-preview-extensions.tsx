...
import { highlightCode, highlightToTokens, getDisplayName } from '@profer/core'
import { MermaidBlock } from '@profer/ui'
...
function isExternalUrl(src: string): boolean {
  return /^(?:https?:|data:|blob:|file:|profer-file:)/i.test(src)
}
...
function resolveMediaSrc(src: string, fileAccessRef: FileAccessRefOrNull, apply: (src: string) => void): () => void {
  // 外链 / data-URL / blob / 已授权 profer-file 协议：直接 apply，不走 IPC
...
function createShikiCodeBlockView(initialNode: ProseMirrorNode, view: EditorView) {
  ...
  copyBtn.addEventListener('click', () => {
    void writeClipboardText(currentCode).then((ok) => {
      if (!ok) return
