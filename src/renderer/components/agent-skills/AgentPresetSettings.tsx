...
export function AgentPresetSettings({ workspaceSlug, search = '' }: { workspaceSlug?: string; search?: string }): React.ReactElement {
  ...
  // 预设导出/导入（JSON 文件）与跨工作区导入：平板暂不支持（需求边界，按钮已隐藏，stub 拒绝）
  ...
  return (
    <SettingsSection title="Agent 预设" description="预设 = 岗位 + 工作环境：把提示词、推理档位、权限模式、Skill/MCP 白名单与能力裁剪组合成可复用配置。预设为工作区级配置；会话内可随时切换（下一轮消息完整生效），星标为新建会话的默认预设。">
