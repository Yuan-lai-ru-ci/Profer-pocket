...
export function ExpandedSidebar({ s }: { s: SidebarModel }): React.ReactElement {
  ...
  // Pocket 的抽屉不是 macOS 窗口标题栏，不应继承桌面端为红绿灯预留的 30px 空白。
  // 在桌面浏览器模拟移动端时也保持与真实 Android WebView 一致。
  const showMacChrome = isMac && !pocketMode

  return (
    <div className="relative h-full flex flex-col overflow-hidden">
      <SidebarWindowDragStrip
        height={showMacChrome ? SIDEBAR_DRAG_STRIP_HEIGHT.expandedMac : SIDEBAR_DRAG_STRIP_HEIGHT.expanded}
      />

      {/* macOS 需要避开左上角红绿灯；边栏覆盖全局标题栏拖拽层，因此留白自身也要可拖拽。 */}
      <div className={cn('w-full flex-shrink-0 titlebar-drag-region', showMacChrome ? 'h-[30px]' : 'h-1')} />
