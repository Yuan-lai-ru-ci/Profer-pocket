...
export function PocketUpdateCard({ store }: { store: Store }): React.ReactElement | null {
  ...
  return (
    <aside className="fixed right-3 top-[max(0.75rem,env(safe-area-inset-top))] z-[90] w-[min(22rem,calc(100vw-1.5rem))] rounded-xl border border-border bg-background p-3 shadow-xl" aria-label="应用更新">
      <button className="absolute right-2 top-2 rounded p-1 text-muted-foreground hover:bg-muted hover:text-foreground" onClick={() => downloading ? hidePocketUpdateCard(store) : deferPocketUpdate(store)} aria-label={downloading ? '隐藏下载提示' : '稍后提醒'}><X className="size-4" /></button>
      <div className="pr-6 text-sm font-medium">{downloaded ? '更新已就绪' : downloading ? '正在下载更新' : failed ? '更新操作失败' : '发现新版本'}</div>
      <p className="mt-1 text-xs text-muted-foreground">v{update.versionName}{failed ? `：${status.message}` : downloaded ? ' 已验证，可交给系统安装。' : ' 已发布。'}</p>
      {downloading && <div className="mt-3 space-y-1.5"><div className="h-1.5 overflow-hidden rounded bg-muted"><div className="h-full bg-primary transition-all" style={{ width: `${Math.max(0, Math.min(100, status.progress.percent))}%` }} /></div><div className="flex justify-between text-[11px] text-muted-foreground"><span>{status.progress.total > 0 ? `${formatBytes(status.progress.transferred)} / ${formatBytes(status.progress.total)}` : `${formatBytes(status.progress.transferred)} 已下载`}</span><span>{status.progress.total > 0 ? `${Math.round(status.progress.percent)}%` : '下载中'}</span></div></div>}
