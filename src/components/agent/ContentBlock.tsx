...
import { thinkingExpandedAtom } from '@/atoms/ui-atoms'
...
// ===== 格式化耗时（从 AgentMessages 内联，避免循环依赖） =====
...
function ToolUseBlock({ block, allMessages, animate = false, index = 0, dimmed = false, childBlocks, basePath, isStreaming }: ToolUseBlockProps): React.ReactElement {
  ...
  // 移动端：无文件预览能力，不再计算 isPreviewable / 渲染 PreviewOpenButton（依赖 Electron IPC）
...
/** 生成图片缩略图组件（点击可预览大图） */
/** 生成图片缩略图（移动端降级：无 Electron 文件读取能力，渲染为占位块） */
function GeneratedImageThumb({ image }: { image: ParsedGeneratedImage }): React.ReactElement {
  return (
    <div className="flex h-[120px] w-full max-w-[300px] items-center justify-center gap-1.5 rounded-lg border border-dashed border-border/50 bg-muted/20">
      <Download className="size-4 text-muted-foreground/40" />
      <span className="text-xs text-muted-foreground/50">{image.filename}</span>
    </div>
  )
}
