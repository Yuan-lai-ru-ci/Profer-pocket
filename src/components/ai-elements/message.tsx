...
const MarkdownLink = React.memo(function MarkdownLink({
  ...
  return (
    <a
      ...
      onClick={(e) => {
        e.preventDefault()
        // 移动端：无 MainArea 预览面板 / Electron IPC / 系统文件打开能力，
        // 外部 http/https 链接用 window.open 发外部跳转，文件路径点击不做动作。
        if (!href) return
        if (href.startsWith('http://') || href.startsWith('https://')) {
          window.open(href, '_blank', 'noopener,noreferrer')
          return
        }
        // file:// / 本地路径：移动端无文件系统访问，不动作
      }}
      title={href}
    >
      {linkChildren}
...
const MarkdownPre = React.memo(function MarkdownPre({
  ...
  if (codeChild) {
    ...
    // 移动端：Mermaid 图降级为普通代码块（不引 mermaid 渲染引擎），
    // 也不再做语言自动检测（无 highlight.js/shiki），保留语言标签直接交给 CodeBlock。
  }
